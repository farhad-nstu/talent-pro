...
class AuthSessionController extends Controller
{
    ...
    {
        return $this->admin_guard()->attempt(
    ...
    protected function credentials(Request $request)
    {
        if (is_numeric($request->email)) {
            return ['mobile' => $request->email, 'password' => $request->password, 'status' => 1];
        } else {
    ...
    protected function sendLoginResponse(Request $request)
    {
        ...
        if ($response = $this->authenticated($request, $this->admin_guard()->user())) {
            return $response;
        }
